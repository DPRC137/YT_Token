...
from ytTok import Main, add_purchase_time_annotation, find_valid_assets, is_valid_address
...
class TestytTokValidation(unittest.TestCase):
    ...
    def test_main_with_valid_contracts(self, mock_st_error):
        ...
        self.assertEqual(
            main.url_apy, f"https://api-v2.pendle.finance/core/v1/1/markets/{market_contract}/apy-history-1ma"
        )
        self.assertEqual(main.url_ohlcv_yteth, f"https://api-v2.pendle.finance/core/v3/1/prices/{yt_contract}/ohlcv")
...
def test_unsupported_network_stops_execution():
    with (
        patch("streamlit.selectbox", return_value="invalid_network"),
...
def test_no_valid_assets_stops_execution():
    ...
    st.cache_data.clear()
    with (
        patch("streamlit.selectbox", return_value="ethereum"),
...
class TestOHLCV(unittest.TestCase):
    ...
    def test_missing_volume_key(self):
        st.cache_data.clear()
        results = [{"time": "2023-01-01T00:00:00.000Z", "open": 1.0, "high": 2.0, "low": 0.5, "close": 1.5}]
        self.main_obj.session = MockSession(results)
        df_opt = self.main_obj.fetch_yteth_ohlcv()
...
def test_find_valid_assets_matching_item():
    data = [{"baseType": "YT", "address": "0x123", "expiry": "2024-12-31", "symbol": "TEST"}]
...
def test_find_valid_assets_no_matches():
    data = [{"baseType": "PT", "address": "0x123"}, {"baseType": "YT", "address": "0x456", "expiry": "2025-01-01"}]
    result = find_valid_assets(data, "YT", "expiry", "0x789")
    assert result == []


def test_find_valid_assets_additional_keys():
    data = [{"baseType": "YT", "address": "0x123", "expiry": "2024-12-31", "extra_key_1": "foo", "extra_key_2": 123}]
    result = find_valid_assets(data, "YT", "expiry", "0x123")
    assert len(result) == 1
...
def test_main_class_methods_pass_timeout():
    ...
    # Test fetch_apy
    st.cache_data.clear()
    mock_response.json.return_value = {"results": "timestamp,impliedApy,underlyingApy\n1700000000,0.05,0.06"}
    main_obj.fetch_apy()
    assert mock_session.get.called
    _, kwargs = mock_session.get.call_args
    assert kwargs.get("timeout") == 10
